...
class Kohana_ORM {
	...
	public function __call($method, array $args)
	{
        ...
        {
            return array_key_exists($column, $this->_original_data)
                    ? $this->_original_data[$column]
                    : $this->__get($column);
        }
	...
	public function __get($column)
	{
		...
		elseif (isset($this->_belongs_to[$column]))
		{
			...
			$col = $model->_table_name.'.'.$model->_primary_key;
                        ...
                        //POZOR: Nasledujici radek zmenen
                        //je to kvuli tomu abych mohl dynamicky pretizit hodnotu
                        //ciziho klice v __get metode v konkretni instanci modelu
                        //$val = $this->_object[$this->_belongs_to[$column]['foreign_key']];
			$val = $this->{$this->_belongs_to[$column]['foreign_key']};
	...
	public function __set($column, $value)
	{
		...
		{
                        if ( ! array_key_exists($column, $this->_original_data))
                        {
                            $this->_original_data[$column] = $this->_object[$column];
                        }
	...
	public function save()
	{
		...
		if ($this->_saved === TRUE)
		{
			...
			$this->_original_data = $this->_changed = array();
		}
	...
	public function pk()
	{
            if ($this->_primary_key == 'id') {
                $this->_primary_key = $this->_table_name.'id';
            }
