...
class AppFormItem_RelNNSelect extends AppFormItem_Base
{
    ...
    protected $config = array(
        ...
        // Enable option to add a note to each association
        'note' => false,

        // Name of an additional column which can be set for each association (similar as note)
        'additional_data' => null,

        // Can be used to override name of the pivot table (otherwise it's generated by Helper_FormItem::NNTableName)
        'nntablename' => null,

        // If set, a link to object overview page will be displayed with given text
        'link' => null,

        // Can be used to overload items' preview format (eg. '@name @price')
        'preview_format' => null
    );
    ...
    public function __construct($attr, $config, Kohana_ORM $model, ORM_Proxy $loaded_model, $form_data, $form)
    {
        ...
        if ($this->config['nntablename']) {
            $this->map = $this->config['nntablename'];
        } else {
    ...
    public function init()
    {
        ...
        if (arr::get($this->config, 'note') or arr::get($this->config, 'additional_data') or arr::get($this->config, 'allow_check_all') or arr::get($this->config, 'fast_search')) {
    ...
    protected function getRelItems()
    {
        ...
        else
        {
            ...
            if (arr::get($this->config, 'note') || arr::get($this->config, 'additional_data')) {
            ...
            foreach ($map_data as $model)
            {
                ...
                if (arr::get($this->config, 'additional_data')) {
                    $additional_data[$model->{$rel_key}] = $model->{$this->config['additional_data']};
                }
            }

            return Array('id' => $id, 'note' => $note, arr::get($this->config, 'additional_data') => $additional_data);
        }
    }
    ...
    protected function addRelItem($rel, $model, $data=NULL)
    {
        ...
        elseif (arr::get($this->config, 'note') || arr::get($this->config, 'additional_data')) {
    ...
    public function processFormEvent($type, $data)
    {
        switch($type)
        {
            ...
            case AppForm::FORM_EVENT_AFTER_SAVE:
                ...
                $additional_data = (array)arr::get((array)$this->form_data, $this->config['additional_data']);
                ...
                foreach ($data_id as $relitemid)
                {
                    ...
                    if (arr::get($this->config, 'note')) {
                        $data['note'] = trim(arr::get($data_note, $relitemid, ''));
                    }

                    if (arr::get($this->config, 'additional_data')) {
                        $data[$this->config['additional_data']] = trim(arr::get($additional_data, $relitemid, ''));
                    }
    ...
    public function Render($render_style = NULL, $error_message = NULL) {
        ...
        $view->additional_data = arr::get($this->config, 'additional_data');

        $view->link = arr::get($this->config, 'link');

        $view->preview_format = arr::get($this->config, 'preview_format');

        if (arr::get($this->config, 'additional_data')) {
            $view->additional_data_label = arr::get($this->config, 'additional_data_label');

            if ($view->additional_data_label === NULL) {
                $view->additional_data_label = FormItem::getLabel($this->model->table_name(), $this->config['additional_data']);
            }
        }
